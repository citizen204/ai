...
describe('Grok Video Adapter', () => {
  describe('factories', () => {
    it('creates an adapter with the provided API key', () => {
      ...
      expect(adapter.model).toBe('grok-imagine-video')
    })

    it('supports the 1.5 preview model', () => {
      const adapter = createGrokVideo(
        'grok-imagine-video-1.5-preview',
        'test-api-key',
      )
      expect(adapter.model).toBe('grok-imagine-video-1.5-preview')
    })
  ...
  describe('createVideoJob', () => {
    it('posts a JSON request to the Imagine generations endpoint', async () => {
      ...
      expect(result).toEqual({ jobId: 'req-123', model: 'grok-imagine-video' })
    ...
    it('passes modelOptions through, including the image-to-video starting frame', async () => {
      const fetchMock = mockFetch(() => jsonResponse({ request_id: 'r' }))
      const adapter = adapterWithFetch(fetchMock)
    ...
    it('honours a custom baseURL', async () => {
      const fetchMock = mockFetch(() => jsonResponse({ request_id: 'r' }))
      const adapter = createGrokVideo('grok-imagine-video', 'k', {
